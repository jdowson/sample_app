...
describe MicropostsController do
  ...
  describe "GET 'index'" do
	...
	i = 1
        ...
        50.times do
          @microposts << Factory(:micropost, :user => @user, :content => "Content#{i}")
          i += 1
        end

        @microposts.should_receive(:paginate).and_return(@microposts.paginate)
      ...
      it "should have the right title" do
        get :index, :user_id => @user.id
        response.should have_tag("title", /#{@user.name}/i)
      end

      it "should have an element for each micropost" do
        get :index, :user_id => @user.id
        @microposts[0..2].each do |micropost|
          response.should have_tag("span", micropost.content)
        end
      end
      ...
      it "should paginate microposts" do
        ...
        response.should have_tag("a[href=?]", "/users/#{@user.id}/microposts?page=2", "2")
        response.should have_tag("a[href=?]", "/users/#{@user.id}/microposts?page=2", "Next &raquo;")
      end
